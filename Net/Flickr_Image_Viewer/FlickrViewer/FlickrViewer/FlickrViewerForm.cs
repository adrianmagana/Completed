...
namespace FlickrViewer
{
   public partial class FlickrViewerForm : Form
   {
      ...
      // http://www.flickr.com/services/apps/create/apply
       private const string KEY = "fa2e36e5a2f8f1ca83c4cd7e8bccd383";
      ...
      private async void searchButton_Click( object sender, EventArgs e )
      {
        ...
        //privacy_filter=1. Use the inputTextBox.Text.Replace(" ", ",") for tag.
         var flickrURL = string.Format( "https://api.flickr.com/services/rest/?method=flickr.photos.search&tag_mode=all&per_page=500&api_key={0}&tags={1}", KEY, inputTextBox.Text.Replace(" ",","));
         ...
         try
         {
            ...
            XDocument xDoc =XDocument.Parse(await flickrTask);
            ...
            var flickrPhotos =
               ...
               let id = (string)photo.Attribute("id")
               let title = (string)photo.Attribute("title")
               let secret = (string)photo.Attribute("secret")
               let server = (string)photo.Attribute("server")
               let farm = (string)photo.Attribute("farm")
         ...
         object sender, EventArgs e )
      {
         if ( imagesListBox.SelectedItem != null )
         {
            ...
            //Invoke the the WeClient DownloadDataTaskAsync method to get byte array called imageBytes that contains the photo (from selectedURL) and await the results. 
             Byte[] imageBytes= await client.DownloadDataTaskAsync( new Uri(selectedURL));
